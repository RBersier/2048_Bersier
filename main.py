...
Titre : Jeu 2048
Auteur : Ryan Bersier
Date de dernière modification : 06.04.23
Version : 1.0
"""
...
# check les mouvements disponibles pour la grille
...
def highscore():
    global nb_highscore
    # verifies si le score est supérieur au highscore
...
# actualise-les différentes stats présente sur la partie graphique pendant la partie
...
# gère l'apparition de 2 et de 4
def random_nb():
    # definition d'une variable pour que si aucune case n'est disponible qu'il ne tente pas d'en régénérer des 2 ou des 4
    i = 0
    if movement_checker():
        # definition d'une variable pour avoir 10% de chance qu'un 4 se génère
        x = random.randint(0, 9)
        while True:
            # choisie une case aléatoire parmis les 16 disponibles
            ...
            if i == 0:
                ...
                # verifies que la case sélectionner est vide
                if table2[y][z] == "":
                    # ajout de 4
                    if x == 0:
                        table2[y][z] = power2[1]
...
# pour les démarage
def start_game():
    # si la grille est vide généré 2 nombre sur la grille
    if table2 == [["", "", "", ""], ["", "", "", ""], ["", "", "", ""], ["", "", "", ""]]:
        random_nb()
        random_nb()
...
def reset_game(window=None):
    ...
    # redéfinissions des variables stat à 0 (sauf highscore)
    nb_score = 0
    nb_move = 0
    # vide la variable contenant la grille et la redéfinie avec rien dedans
    table2.clear()
    table2 = [["", "", "", ""], ["", "", "", ""], ["", "", "", ""], ["", "", "", ""]]
    refresh()
    # pour la partie gagner detruit la fenêtre parallèle pour si le joueur choisit de recommencer une partie
...
def win_game():
    ...
    won = False
    # verifies sur toutes les cases la présence d'une case 2048 si oui passe la variable gagné à vrai
    for line in range(len(table2)):
        for col in range(len(table2[line])):
            if table2[col][line] == power2[10] and w == 0:
                won = True
    # si gagné créer une fenêtre au-dessus de la première
    if won:
...
# fonction pour la partie perdue
def lose_game():
    # raccourci pour la fonction mouvement checker
    mc = movement_checker()
    # verifies si plus aucun mouvement disponible
    if not mc:
...
# définition de la fenêtre et ces paramètres (le code ne peux pas fonctionner si cette partie se retrouve dans une fonction)
